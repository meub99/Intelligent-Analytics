    ...
    internal class SentimentEventProcessor : IEventProcessor
    {
        ...
        async Task IEventProcessor.ProcessEventsAsync(PartitionContext context, IEnumerable<EventData> messages)
        {
            foreach (var eventData in messages)
            {
                try
                {
                    ...
                    msgObj.score = await GetSentimentScore(msgObj.message);  //invokes GetSentimentScore and sets the score on the msg object
        ...
        private async Task<double> GetSentimentScore(string messageText)
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                var jsonreq = JsonConvert.SerializeObject(req); //complete this...  
                byte[] bytedata = Encoding.UTF8.GetBytes(jsonreq); //complete this...get byte array from jsonreq 
...

                {
                    ...
                    var sentimentResponse = await client.PostAsync(uri, content); //Complete this... do a Post using the uri and content 
                    jsonResponse = await sentimentResponse.Content.ReadAsStringAsync(); //Complete this...extract the response content as a string 
                }
                Console.WriteLine("\nDetect sentiment response:\n" + jsonResponse);
                    ...
                    sentimentScore = result.documents[0].score; //Complete this...retrieve the score for the first document in the result
